...
async def linkable_merge_requests(
    work_item_id: uuid.UUID, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)
) -> list[dict]:
    """Open MRs in the item's project that aren't already linked to it; the user's own first."""
    item = await db.get(WorkItem, work_item_id)
    if item is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Work item not found")

    result = await db.execute(
        select(MergeRequest, User.name)
    ...
    rows = [
        ...
        for mr, author_name in result.all()
    ]
    return sorted(rows, key=lambda r: not r["mine"])
...
async def link_merge_request(
    ...
    item, mr, project = await _item_project_and_mr(db, work_item_id, body.merge_request_id)
    access_token = await get_write_access_token(db, current_user)
    try:
        title, description = await update_merge_request_references(
            project, mr.gitlab_iid, item.gitlab_iid, access_token, link=True, closes=body.closes
        )
    ...
    return {
        ...
        "note": None if linked else "GitLab accepted the change but the link was not detected; it will appear after the next sync.",
    }
